...
export function MatchScore({ score, isLoading = false, errorText }: MatchScoreProps) {
  if (isLoading) {
    return (
  ...
  if (errorText || score === undefined) {
    return (
      <div className="flex items-center gap-2 w-40">
        ...
        <span className="text-helper text-muted">{errorText ?? 'Unavailable'}</span>
      </div>
    )
  }
  ...
  return (
    <div className="flex items-center gap-2 w-40" role="meter" aria-valuenow={Math.round(score * 100)} aria-valuemin={0} aria-valuemax={100}>
      <div className="h-2 flex-1 rounded-sm bg-border overflow-hidden">
        <div className={`h-full ${barColor[band]}`} style={{ width: `${Math.round(score * 100)}%` }} />
      </div>
